...
- 화면을 좌우로 나눠서 **왼쪽에는 입력**(유튜브 URL, 재료 사진), **오른쪽에는
  영수증 결과**가 나오도록 구성했습니다.
- 오른쪽 영역은 화면 높이에 고정(`position: sticky`)되어 있고, 그 안의
  영수증 목록만 자체적으로 스크롤됩니다 (왼쪽 입력 폼은 페이지와 함께
  스크롤). 900px 이하 화면에서는 위/아래로 쌓이는 1단 레이아웃으로
  자동 전환됩니다.
- 기존 레시피 영수증과 대체 레시피 영수증은 **서로 다른 영수증 카드**로
  분리되어 있습니다. 대체 레시피 카드는 위쪽에 "대체 레시피" 리본이
  붙어 있고 테두리 색(mustard)도 달라서 한눈에 구분됩니다. 대체
  가능한 재료가 없으면 대체 레시피 카드는 아예 나타나지 않습니다.
